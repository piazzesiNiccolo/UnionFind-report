...
\paragraph{Path compression} make all the nodes $u_i$ ($ 0 \leq i \leq l-3$) children of the root $u_{l-1}$
\begin{lstlisting}
    int find(int x){
        int p = x;
        if (parents[p] != p)
...
\end{lstlisting}
\paragraph{Path splitting} make the node $u_i (0 \leq i \leq l-3)$ a child of its grandparent $u_{i+2}$ 
\begin{lstlisting}
